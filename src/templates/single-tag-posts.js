...
const singleTagPosts = ({ data, pageContext }) => {
  ...
  return (
    <Layout>
      <Helmet>
        <title>
          Posts by #{tagName} | {data.site.siteMetadata.title}
        </title>
      </Helmet>
      <div className="uk-width-1-1 uk-width-2-3@l uk-width-2-3@xl uk-align-center">
        <h2 className="uk-article-title uk-margin-large-bottom">
          Posts by #{tagName}
        </h2>
        {posts.map((post, index) => {
          return (
            <div
              className="uk-card uk-card-default uk-margin-bottom uk-grid"
              key={index}
            >
